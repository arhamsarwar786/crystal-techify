...
export function Logo({
  ...
  const mark = (
    <Image
      src="/brand/mark.png"
      alt={variant === "mark" ? "Crystal Techify" : ""}
      width={512}
      height={530}
      priority={priority}
  ...
  return (
    <span className={`inline-flex items-center gap-2.5 ${className ?? ""}`}>
      ...
      <span className="flex flex-col leading-none">
        <span className="font-display text-[15px] font-semibold tracking-[0.16em] text-ink">
          CRYSTAL
        </span>
        <span className="gradient-text font-display text-[10px] font-medium tracking-[0.44em]">
          TECHIFY
        </span>
      </span>
    </span>
